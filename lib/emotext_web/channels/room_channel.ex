defmodule Emotext.Web.RoomChannel do
  use Phoenix.Channel
  ...
  def action_str(str, user) do
    if (str != nil) do
      ...
      String.replace(str, "$n", user.screen_name);
    end
  end
  ...
  def action_str(str, user, vict) do
      if (str != nil) do
        ...
        String.replace(str, "$N", vict.screen_name);
    #   cond do
    #     true ->
  ...
  defp save_action(action, user, vict \\ nil) do
      ...
      if vict do
        Repo.insert! Ecto.Changeset.change(changeset, %{ vict_id: vict.id, vict_screen_name: vict.screen_name })
      else
